// Quick test with first conversation only
...
const dbPath = '/tmp/locomo-quick-test2.db';
...
console.log('Testing first 20 questions:\n');

for (let i = 0; i < Math.min(20, conv.qa.length); i++) {
  const qa = conv.qa[i];
