...
export class MuninnDatabase {
  ...
  async findOrCreateEntity(name: string, type: string, summary?: string): Promise<Entity> {
  ...
  async createEpisode(episode: Omit<Episode, 'id' | 'ingestedAt'>): Promise<Episode> {
    const result = await this.pool.query<Episode>(`
  ...
  async createFact(fact: Omit<Fact, 'id' | 'createdAt'>): Promise<Fact> {
    const result = await this.pool.query<Fact>(`
      INSERT INTO facts (
  ...
  async getCurrentFacts(entityName: string, predicate?: string): Promise<Fact[]> {
  ...
  async invalidateFact(factId: string, reason?: string): Promise<void> {
  ...
  async createEvent(event: Omit<Event, 'id' | 'observedAt'>): Promise<Event> {
  ...
  async getEntityEvolution(entityName: string, from?: Date, to?: Date): Promise<Event[]> {
    const result = await this.pool.query(`
  ...
  async createRelationship(rel: Omit<Relationship, 'id' | 'createdAt'>): Promise<Relationship> {
    const result = await this.pool.query<Relationship>(`
  ...
  async traverseGraph(startEntity: string, maxDepth: number = 3): Promise<any[]> {
    const result = await this.pool.query(`
  ...
  async createContradiction(contradiction: Omit<Contradiction, 'id' | 'detectedAt'>): Promise<Contradiction> {
    const result = await this.pool.query<Contradiction>(`
