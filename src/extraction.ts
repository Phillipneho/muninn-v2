...
export class FactExtractor {
  ...
  async extract(content: string, sessionDate?: string): Promise<ExtractionResult> {
    ...
    try {
      const result = JSON.parse(text) as ExtractionResult & { speaker?: string };
  ...
  private validateAndClean(result: ExtractionResult & { speaker?: string }): ExtractionResult {
    return {
...
export function resolveEntities(
  ...
  existing: Map<string, { id: string; type: string; aliases: string[] }>
): Map<string, { id: string; confidence: number; isNew: boolean }> {
  const resolved = new Map<string, { id: string; confidence: number; isNew: boolean }>();
...
export function detectContradictions(
...
): Array<{ fact: ExtractedFact; type: 'value_conflict' | 'temporal_overlap' | 'logical'; stateChange?: { oldValue: string; newValue: string }; isTransient?: boolean }> {
  const contradictions: Array<{ fact: ExtractedFact; type: 'value_conflict' | 'temporal_overlap' | 'logical'; stateChange?: { oldValue: string; newValue: string }; isTransient?: boolean }> = [];
  ...
  for (const existing of existingFacts) {
    ...
    const existingObj = (existing as any).object?.toLowerCase() || '';
    ...
    if (
      ...
      const isSubstringTransient = !isExactPersistent && TRANSIENT_PATTERNS.some(t => 
        t.length > 2 && predicate.includes(t)
      );
      
      const isSubstringPersistent = PERSISTENT_PATTERNS.some(p => 
        p.length > 2 && predicate.includes(p)
      );
